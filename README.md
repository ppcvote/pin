...
- **ATR threat scan at skill load** — every `SKILL.md` is scanned against the open [agent-threat-rules](https://github.com/Agent-Threat-Rule/agent-threat-rules) pack (the same ruleset running in Microsoft AGT and Cisco AI Defense) before it enters the registry. A critical-severity match refuses the skill; CI re-runs the scan on every PR touching `skills/` (SARIF → Security tab). Opt out per env with `PIN_ATR_SCAN=false`.
